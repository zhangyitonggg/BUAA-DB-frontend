                                ...
                                <v-col>
                                    <v-chip-group v-model="filters.tags" multiple column active-class="active-tag">
                                        <v-chip v-for="(tag, index) in availableTags" :key="index"
                                            :color="getTagColor(tag)" outlined @click="toggleTag(tag)">
                                            {{ tag }}
                                        </v-chip>
                                    </v-chip-group>
                                </v-col>
                ...
                <div class="container">
                    <div class="row">
                        <div class="card" v-for="(item, index) in cards" :key="index">
                            ...
                            <div class="card-tags">
                                <v-chip-group column>
                                    <v-chip v-for="(tag, idx) in item.tags" :key="idx" :color="getTagColor(tag)"
                                        outlined>
                                        {{ tag }}
                                    </v-chip>
                                </v-chip-group>
...
export default {
    ...
    mounted() {
        this.$store.commit("setAppTitle", "管理悬赏");
        this.getTasks();
    },
};
</script>
...
.card {
    position: relative;
    /* 确保 .action-buttons 以 card 为参考定位 */
    flex: 0 0 48%;
    /* 每行展示三个卡片 */
    margin-bottom: 20px;
    padding: 15px;
